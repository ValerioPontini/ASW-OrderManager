...
import asw.ordermanager.productservice.eventpublisher.ProductCreatedEventKafkaPublisher;
import asw.ordermanager.productservice.eventpublisher.ProductStockLevelUpdatedEventKafkaPublisher;
...
public class ProductService {
	...
	@Autowired
    private ProductCreatedEventKafkaPublisher productEventKafkaPublisher;

    @Autowired
    private ProductStockLevelUpdatedEventKafkaPublisher productStockLevelUpdatedEventKafkaPublisher;
    ...
    private final Logger logger = Logger.getLogger(this.getClass().toString());
 	...
 	public Product createProduct(String name, String category, int stockLevel, double price) {
		...
		productEventKafkaPublisher.publish(event);
		return product;
	}
	...
	public Collection<Product> getProductsByNames(List<String> names) {
 	...
 	public Product updateProductStockLevel(String name, int stockLevelVariation) { 
		...
		product = productRepository.save(product);
		DomainEvent event = new ProductStockLevelUpdatedEvent(name, stockLevelVariation);
		productStockLevelUpdatedEventKafkaPublisher.publish(event);
		return product;
	}
